...
for sample_name, sample_dir in input_dirs.items():
	...
	index[sample_name] = [os.path.normpath(x) for x in index[sample_name] if os.path.getsize(x) > 0]
	index[sample_name] = index[sample_name][:max_files]

	relindex[sample_name] = [x.replace(v1p0pre7dir, "") for x in index[sample_name]]
pprint(relindex)
...
# Tar
relfiles = []
for filelist in relindex.values():
	relfiles.extend(filelist)
relfiles_str = " ".join(relfiles)
tar_command = f"tar -czvf /project/users/davidryu/yoda_v1.0pre7.tar.gz -C {v1p0pre7dir} {relfiles_str}"
